...
c
c       Cyclical GMRES algorithm, recalculate the residual every 10 steps
c
...
c
cccc        write(*,*) 'in cgmres, iter=', iter
c     
c       ... restart GMRES
c
        if( m .eq. ngmrec ) then
...
c
        d=sqrt(d/n)
        errs(niter)=dble(d)
cccc        write(*,*) 'in cgmres, error=', errs(niter)
c
        if (ifprint .eq. 1) then
            ii2=iter
            err2=errs(niter)
            write(*,*) 'in cgmres, iter=',ii2,', error=', err2
        endif
c
        if( abs(d) .lt. eps ) return
c
